...
abstract contract RoomRental is IRoom, Ownable {
    ...
    function signForRental(
        ...
        require(
            msg.value >=
                _rooms[roomUid].rentAmountPerMonth +
                    _rooms[roomUid].depositAmount,
            "!balance for sign"
        );
        ...
        emit TransferBalance(
            _rooms[roomUid].renter,
            _rooms[roomUid].owner,
            _rooms[roomUid].rentAmountPerMonth,
            TransferAction.Sign
        );
        emit RentStarted(
    ...
    function payForRent(
        ...
        require(
            msg.value >= _rooms[roomUid].rentAmountPerMonth,
            "!balance for rent"
        );
    ...
    function endRentalRoomWithPenaltyFee(
        ...
        require(
            msg.sender == _rooms[roomUid].owner &&
                msg.sender == _rooms[roomUid].renter,
            "!no access"
        );
        ...
        if (msg.sender == _rooms[roomUid].renter) {
            ...
            _rooms[roomUid].renter.transfer(
                _rooms[roomUid].depositAmount + penaltyFee
            );
            emit TransferBalance(
                address(this),
                _rooms[roomUid].renter,
